...
def calculate_mterics(live, trained_models, data, split="test"):
    ...
    for model in trained_models:
        ...
        if not model.model_type in live.summary["avg_prec"] :
            live.summary["avg_prec"][model.model_type] = {}
            live.summary["roc_auc"][model.model_type] = {}
        live.summary["avg_prec"][model.model_type][split] = avg_prec
        live.summary["roc_auc"][model.model_type][split] = roc_auc
...
def main():
    cfg = ConfigManager().config #["evaluation"]
    ...
    live = Live(cfg["evaluation_folder"])
